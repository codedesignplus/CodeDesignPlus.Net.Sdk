...
public class RabitMQExceptionTest
{
    ...
    [Fact]
    public void RabitMQException_SerializationInfo_Call_Method()
    {
        // Arrange
        var errors = new List<string>() { Guid.NewGuid().ToString() };
        var message = Guid.NewGuid().ToString();
        ...
        var deserialize = JsonConvert.DeserializeObject(serialize, typeof(RabitMQException)) as RabitMQException;
